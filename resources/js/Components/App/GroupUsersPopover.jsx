...
export default function GroupUsersPopover({ users = [], authUser }) {
    // Filtra para excluir al usuario autenticado
    const filteredUsers = users.filter((u) => u.id !== authUser.id);
    
    console.log("Usuario autenticado en GroupUsersPopover:", authUser.id);
    console.log("Usuarios filtrados:", filteredUsers);  // Agrega este log para ver la lista filtrada
                        ...
                        <Popover.Panel className="absolute right-0 z-20 mt-3 w-[220px] px-4 sm:px-0">
                            <div className="overflow-hidden rounded-xl shadow-xl backdrop-blur bg-gray-900/80 ring-1 ring-white/10">
                                <div className="py-2">
                                    ...
                                    {/* Cambia a filteredUsers.map para aplicar el filtro visualmente */}
                                    {filteredUsers.map((user, i) => (
                                        <div key={user.id}>
                                            ...
                                            {/* Cambia a filteredUsers.length - 1 para que el separador funcione con la lista filtrada */}
